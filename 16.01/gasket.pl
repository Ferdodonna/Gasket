...
ergaenzen(kreis(P1,R1),kreis(P2,R2),R3,P3A,P3B) :-
  H is (2/(R1+R2)) * sqrt( (R1 + R2 + R3) * R1 * R2 * R3 )
  , P1 = kartesisch(X1,Y1)
  , P2 = kartesisch(X2,Y2)
  , D is R1 + R2
  , AX is (X2 - X1) * R1 / D + (Y2 - Y1) * H / R1 + X1
  , AY is (Y2 - Y1) * R1 / D + (X1 - X2) * H / R1 + Y1
  , BX is (X2 - X1) * R1 / D + (Y1 - Y2) * H / R1 + X1
  , BY is (Y2 - Y1) * R1 / D + (X2 - X1) * H / R1 + Y1
  , P3A = kartesisch(AX,AY)
  , P3B = kartesisch(BX,BY)
.

eg(kreis(P1,R1),kreis(P2,R2),R3,P3A,P3B) :-
  A is R2+R3
  , B is R3+R1
  , C is R1+R2
  , WBAC is acos((A*A - B*B - C*C) / (-2 * B * C))
  , abstand_vektor(P1,P2,kartesisch(X,Y))
...
zweiterKreis(Erster,K1,K2,K3,Zweiter) :-
  ...
  % Radius berechnen
  maplist(radius,[Erster,K1,K2,K3],Radien)
...
% eine andere Weise das zu berechnen
findCenters(kreis(P1,R1),kreis(P2,R2),kreis(P3,R3),kreis(P5,R5)) :-
  maplist(kehr,[R1,R2,R3],[K1,K2,K3])
  , descartes(K1,K2,K3,_,K5)
  , R5 is 1/K5
  , maplist(scalarProd,[K1,K2,K3],[P1,P2,P3],[Q1,Q2,Q3])
  , maplist(im_mult,[Q1,Q2,Q3],[Q2,Q3,Q1],[Q12,Q23,Q31])
  , foldl(im_add,[Q12,Q23],Q31,Qsums)
  , foldl(im_add,[Q1,Q2],Q3,Qsum)
  , im_sqrt(Qsums,Qsqrt,_)
  , scalarProd(-2,Qsqrt,Q0)
  , im_add(Q0,Qsum,Qr)
  , scalarProd(R5,Qr,P5)
.

% Vektor und Komplexe Zahl Operationen
scalarProd(S,kartesisch(X,Y),kartesisch(SX,SY)) :-
  maplist(is,[SX,SY],[S*X,S*Y]).

% multiplikation komplexer zahlen
im_mult(kartesisch(X,Y),kartesisch(A,B),kartesisch(Q,P)) :-
  P is X*B + Y*A
  , Q is X*A - Y*B
.

% addition komplexer zahlen
im_add(kartesisch(X,Y),kartesisch(A,B),kartesisch(Q,P)) :-
  Q is X+A
  , P is Y+B
.

% absoluter wert einer komplexen zahl
im_abs(kartesisch(X,Y),D) :-
  D is sqrt(X*X + Y*Y)
.

% komplexe zahl in polarschreibweise
im_polar(kartesisch(X,Y),polar(Phi,R)) :-
 im_abs(kartesisch(X,Y),R)
 , Phi is asin(Y/R)
.

% wurzel einer komplexen zahl
im_sqrt(S,kartesisch(A,B),kartesisch(-A,-B)) :-
  im_polar(S,polar(Phi,R))
  , PH is Phi / 2
  , Rs is sqrt(R)
  , A is Rs * cos(PH)
  , B is Rs * sin(PH)
...
% normaler descartes für kreis krümmung
...
compute(1,K1,K2,K3) :-
	K1 = kreis(kartesisch(400, 400),100)
	, K2 = kreis(kartesisch(600,400),100)
	, P3 = kartesisch(500, 600)
	, position(K1,P1)
	, abstand(P1,P3,D)
	, radius(K1, R1)
	, R3 is D-R1
	...
	, write(KU)
  , zweiterKreis(KU,K1,K2,K3,K4)
  , zweiterKreis(K3,KU,K1,K2,K5)
  , zweiterKreis(K2,K3,KU,K1,K6)
  , zweiterKreis(K1,K2,K3,KU,K7)
  , nl, nl
  , print(K4)
  , print(K5)
  , print(K6)
  , print(K7)
  , findCenters(K1,K2,K3,T)
  , print(T)
.
...
drehen(gerade(kartesisch(X,Y),G),Winkel,Gn) :-
  Xn is X + 1
  , Yn is G + Y
  , abstand(kartesisch(X,Y),kartesisch(Xn,Yn),D)
  , Do is D * tan(Winkel)
  , orthogonal(G,Go)
  , move(Do,gerade(kartesisch(Xn,Yn),Go),Pr)
  , gradient(kartesisch(X,Y),Pr,Gn)
.
